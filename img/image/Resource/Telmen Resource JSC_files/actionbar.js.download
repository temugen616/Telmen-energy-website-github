...
function postToApi(path,namespace,body={}){const $=jQuery;var request={path,body,method:'POST',apiNamespace:namespace};$.wpcom_proxy_request(request);}
...
var body=document.body;var lastScrollTop=window.scrollY||window.pageYOffset||0;var fhtml,fbhtml,fbHtmlLi,followingbtn,followbtn,fbdf,slkhtml='',foldhtml='',reporthtml='',customizeIcon,editIcon,statsIcon,themeHtml='',signupHtml='',loginHtml='',viewReaderHtml='',editSubsHtml='',editFollowsHtml='',followBubbleHtml='',actionbar;if(window!=window.top){return;}
				...
				<ul> \
					...
					<div class="actnbr-site-settings"> \
						<div class="actnbr-site-settings__setting"> \
							...
							<label for="inspector-toggle-control-1" class="components-toggle-control__label">'+fbd.i18n.notifyNewPosts+'</label> \
						</div> \
						<p class="actnbr-site-settings__details">'+fbd.i18n.notifyNewPostsDetails+'</p> \
						<div class="actnbr-site-settings__setting"> \
							<span class="actnbr-site-settings__toggle"> \
							...
							<label for="inspector-toggle-control-1" class="components-toggle-control__label">'+fbd.i18n.emailNewPosts+'</label> \
						</div> \
						<div class="actnbr-site-settings__details" id="email-new-posts-details"> \
							<ul class="segmented-control" role="radiogroup"> \
								<li class="segmented-control__item"> \
									<a class="segmented-control__link frequency-instantly">'+fbd.i18n.instantly+'</a> \
								</li> \
								<li class="segmented-control__item"> \
									<a class="segmented-control__link frequency-daily">'+fbd.i18n.daily+'</a> \
								</li> \
								<li class="segmented-control__item"> \
									<a class="segmented-control__link frequency-weekly">'+fbd.i18n.weekly+'</a> \
								</li> \
							</ul> \
						</div> \
						<div class="actnbr-site-settings__setting"> \
							<span class="actnbr-site-settings__toggle"> \
								<input class="actnbr-site-settings__toggle__input" id="toggle-input-email-comments" type="checkbox"></input> \
								<span class="actnbr-site-settings__toggle__track"></span> \
								<span class="actnbr-site-settings__toggle__thumb"></span> \
							</span> \
							<label for="inspector-toggle-control-1" class="components-toggle-control__label">'+fbd.i18n.emailNewComments+'</label> \
						</div> \
					</div> \
				</ul> \
...
var isFollowBubbleOpen=false;var follow=actionbar.querySelector('.actnbr-actn-follow');const notifyPostsToggle=actionbar.querySelector('#toggle-input-notify-posts');const emailPostsToggle=actionbar.querySelector('#toggle-input-email-posts');const emailCommentsToggle=actionbar.querySelector('#toggle-input-email-comments');const frequencyOptions=actionbar.querySelectorAll('.segmented-control__link');const instantlyOption=actionbar.querySelector('.frequency-instantly');const dailyOption=actionbar.querySelector('.frequency-daily');const weeklyOption=actionbar.querySelector('.frequency-weekly');if(notifyPostsToggle){notifyPostsToggle.addEventListener('click',function(e){e.preventDefault();const wrapper=e.target.parentElement;const isEnabling=!wrapper.classList.contains('is-checked');const siteId=fbd.siteID;let restPath='/read/sites/'+siteId+'/notification-subscriptions/'+(isEnabling?'new':'delete');if(isEnabling){e.target.parentElement.classList.add('is-checked');}else{e.target.parentElement.classList.remove('is-checked');}
postToApi(restPath,'wpcom/v2');});}
if(emailPostsToggle){emailPostsToggle.addEventListener('click',function(e){e.preventDefault();const wrapper=e.target.parentElement;const isEnabling=!wrapper.classList.contains('is-checked');const siteId=fbd.siteID;let restPath='/read/site/'+siteId+'/post_email_subscriptions/'+(isEnabling?'new':'delete');let body={};if(isEnabling){e.target.parentElement.classList.add('is-checked');actionbar.querySelector('#email-new-posts-details').classList.add('is-visible');body.delivery_frequency=fbd.subsEmailDefault;frequencyOptions.forEach(function(opt){opt.parentElement.classList.remove('is-selected');});actionbar.querySelector('.frequency-'+fbd.subsEmailDefault).parentElement.classList.add('is-selected');}else{e.target.parentElement.classList.remove('is-checked');actionbar.querySelector('#email-new-posts-details').classList.remove('is-visible');}
postToApi(restPath,'rest/v1.2',body);});}
if(emailCommentsToggle){emailCommentsToggle.addEventListener('click',function(e){e.preventDefault();const wrapper=e.target.parentElement;const isEnabling=!wrapper.classList.contains('is-checked');const siteId=fbd.siteID;let restPath='/read/site/'+siteId+'/comment_email_subscriptions/'+(isEnabling?'new':'delete');if(isEnabling){e.target.parentElement.classList.add('is-checked');}else{e.target.parentElement.classList.remove('is-checked');}
postToApi(restPath,'rest/v1.2');});}
const onFrequencyOptionClick=function(e,frequencyKey){frequencyOptions.forEach(function(opt){opt.parentElement.classList.remove('is-selected');});e.target.parentElement.classList.add('is-selected');const siteId=fbd.siteID;const restPath='/read/site/'+siteId+'/post_email_subscriptions/update';postToApi(restPath,'rest/v1.2',{delivery_frequency:frequencyKey});}
if(frequencyOptions.length>0){instantlyOption.addEventListener('click',function(e){onFrequencyOptionClick(e,'instantly');});dailyOption.addEventListener('click',function(e){onFrequencyOptionClick(e,'daily');});weeklyOption.addEventListener('click',function(e){onFrequencyOptionClick(e,'weekly');});}
follow&&follow.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();isFollowBubbleOpen=true;if(fbd.isLoggedIn){showActionBarStatusMessage('<div class="actnbr-reader">'+fbd.i18n.followedText+'</div>');bumpStat('followed');var eventProps={'follow_source':'actionbar','url':fbd.siteURL};recordTracksEvent('wpcom_actionbar_site_followed',eventProps);followRequest('ab_subscribe_to_blog');const defaultEmailFrequency=fbd.subsEmailDefault;if(defaultEmailFrequency!=='never'){emailPostsToggle.click();}}else{showActionBarFollowForm();}});var unfollow=actionbar.querySelector('.actnbr-actn-following');unfollow&&unfollow.addEventListener('click',function(e){e.preventDefault();unfollow.classList.add('no-display');follow.classList.remove('no-display');bumpStat('unfollowed');var eventProps={'follow_source':'actionbar','url':fbd.siteURL};recordTracksEvent('wpcom_actionbar_site_unfollowed',eventProps);followRequest('ab_unsubscribe_from_blog');const btn=actionbar.querySelector('.actnbr-btn')
btn.classList.add('actnbr-hidden');const followNotificationCheckedToggles=actionbar.querySelectorAll('.actnbr-site-settings__toggle.is-checked');followNotificationCheckedToggles.forEach(t=>{t.classList.remove('is-checked');})});document.addEventListener('click',function(e){const isClickInPopup=!!closest(e.target,'#follow-bubble');if(isClickInPopup){return;}
var btn=actionbar.querySelector('.actnbr-btn');if(isFollowBubbleOpen&&!btn.classList.contains('actnbr-hidden')){isFollowBubbleOpen=false;btn.classList.add('actnbr-hidden');}});var shortlink=actionbar.querySelector('.actnbr-shortlink a');shortlink&&shortlink.addEventListener('click',function(e){e.preventDefault();window.prompt("Shortlink: ",fbd.shortlink);});var ellipsis=actionbar.querySelector('.actnbr-ellipsis');var isMenuOpen=false;ellipsis&&ellipsis.addEventListener('click',function(e){var closestLink=closest(e.target,'a');if(closestLink&&closestLink.classList.contains('actnbr-action')){return false;}
...
function handleScroll(){var scrollTop=window.scrollY||window.pageYOffset||0;var isHidden=actionbar.classList.contains('actnbr-hidden');if(scrollTop<lastScrollTop){actionbar.classList.remove('actnbr-hidden');}else{if(!isHidden&&document.querySelectorAll('#actionbar > ul > li:not(.actnbr-hidden) > .actnbr-popover').length===0){actionbar.classList.add('actnbr-hidden');var menus=actionbar.querySelectorAll('li');for(var i=0;i<menus.length;i++){menus[i].classList.add('actnbr-hidden');}}}
lastScrollTop=scrollTop;}
document.addEventListener('scroll',handleScroll);function recordTracksEvent(eventName,eventProps){eventProps=eventProps||{};window._tkq=window._tkq||[];window._tkq.push(['recordEvent',eventName,eventProps]);}
